...
const RecentlyViewed = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [source, setSource] = useState('recentlyViewed');
  const API_KEY = import.meta.env.VITE_RECIPE_API_KEY;
  ...
  const parseRecentlyViewedIds = () => {
    ...
    const uniqueOrdered = ids.filter((id, index) => ids.indexOf(id) === index);
    ...
    if (savedRecipes) {
      try {
        ...
        if (Array.isArray(parsed)) {
          setRecipes(parsed);
          setSource('recentRecipes');
          setLoading(false);
          return;
        }
      } catch {
        // invalid JSON for recentRecipes, fallback to recentlyViewed flow
      }
    }

    setSource('recentlyViewed');
    fetchRecipeDetails();
  }, []);
  ...
  return (
    <div className="recently-viewed-page">
      <header className="recently-viewed-header">
        ...
        <div className="recently-viewed-stats">
          <span>
            <strong>IDs stored:</strong> {parseRecentlyViewedIds().length}
          </span>
          <span>
            <strong>First ID:</strong> {getRecipeFromIndex(0) || 'N/A'}
          </span>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="recently-viewed-grid">
          {recipes.map((recipe) => (
            <div key={recipe.id} className="recently-viewed-card">
              <img
                src={recipe.image}
                alt={recipe.title}
